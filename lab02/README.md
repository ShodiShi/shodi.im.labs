# 🌡️ Лабораторная работа №2: Имитационное моделирование теплопроводности

## 1. Задание
Реализовать программное приложение на языке Python для моделирования процесса распределения температуры в одномерном стержне (пластине). Основная цель — исследовать влияние шагов дискретизации по пространству ($h$) и времени ($\tau$) на точность и сходимость численного решения уравнения теплопроводности.

## 2. Математическая модель
Движение тепловых потоков в твердом теле описывается дифференциальным уравнением:
$$\rho c \frac{\partial T}{\partial t} = \lambda \frac{\partial^2 T}{\partial x^2}$$

**Метод решения:**
Для реализации выбрана **неявная конечно-разностная схема**, которая обладает абсолютной устойчивостью. Решение системы линейных уравнений на каждом временном шаге выполняется с помощью **метода прогонки (алгоритм Томаса)**:
* **Прямой ход:** расчет прогоночных коэффициентов $\alpha$ и $\beta$.
* **Обратный ход:** определение итоговых температур в узлах сетки.



## 3. Результаты экспериментов
Параметры моделирования для стали: $\rho=7800$, $c=460$, $\lambda=46$. 
Условия: $L=0.1$ м, $T_{left}=200^\circ C$, $T_{right}=20^\circ C$, $T_0=20^\circ C$, время $t=2.0$ с.

| Шаг $\tau$ (с) | Шаг $h$ (м) | Температура в центре (°C) | Время счёта (с) |
| :--- | :--- | :--- | :--- |
| 0.1000 | 0.1000 | 20.0000 | 0.0000 |
| 0.0100 | 0.0100 | 20.0011 | 0.0162 |
| 0.0010 | 0.0010 | 20.0000 | 0.1540 |
| 0.0001 | 0.0001 | 20.0000 | 13.7731 |

## 4. Скриншот работы программы
Приложение визуализирует график температурного профиля и сохраняет историю всех запусков в интерактивной таблице.

<img width="1451" height="973" alt="image" src="https://github.com/user-attachments/assets/a11c0605-374e-4e7d-832d-e553f4003a59" />


## 5. Выводы
* **Сходимость:** При уменьшении шагов $h$ и $\tau$ решение стабилизируется, однако время вычислений при шаге $0.0001$ резко возрастает до ~14 секунд из-за огромного количества итераций.
* **Материал:** Сталь обладает высокой тепловой инерцией, поэтому за 2 секунды тепло от края (200°C) практически не успевает изменить температуру в центре пластины.
* **Стабильность:** Неявная схема показала свою надежность: даже при больших шагах ($dt=0.1$) расчет остается устойчивым и не выдает бесконечных значений.
